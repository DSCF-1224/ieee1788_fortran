program test_is_entire
    ...
    using_set_empty: block
        ...
        if ( is_entire(bare_infsup_real32) ) then
            error stop 'is_entire(bare_infsup_real32 called `set_empty`) must be `.false.`'
        end if


        if ( is_entire(bare_infsup_real64) ) then
            error stop 'is_entire(bare_infsup_real64 called `set_empty`) must be `.false.`'
        end if


        if ( is_entire(bare_infsup_real128) ) then
            error stop 'is_entire(bare_infsup_real128 called `set_empty`) must be `.false.`'
        end if
    ...
    using_set_entire: block
        ...
        if ( .not. is_entire(bare_infsup_real32) ) then
            error stop 'is_entire(bare_infsup_real32 called `set_entire`) must be `.true.`'
        end if


        if ( .not. is_entire(bare_infsup_real64) ) then
            error stop 'is_entire(bare_infsup_real64 called `set_entire`) must be `.true.`'
        end if


        if ( .not. is_entire(bare_infsup_real128) ) then
            error stop 'is_entire(bare_infsup_real128 called `set_entire`) must be `.true.`'
        end if
